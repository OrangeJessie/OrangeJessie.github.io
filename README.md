...
`/knowledge/ai-tools/` 为公开入口，「面试辅导」和「AI教程」模块及文章目录可直接访问。
面试辅导顶部的「历史咨询」默认收起，展开显示公开的咨询反馈和导师证书。
每篇文章有独立链接，进入文章时输入所属模块的文章密码；文章正文使用 PBKDF2-SHA256（600,000 次）和 AES-256-GCM 加密；密码不写入仓库或浏览器存储。
刷新、离开页面或点击「锁定文章」后需要重新输入密码。
...
每份 Markdown 生成一篇独立加密文章，文件名作为链接路径，`title` 元信息作为公开标题。
`draft: true` 的文件不发布；本地模块目录存在时，删除文件会删除该文章，目录为空时清空文章列表。
普通构建仅保留密文时会复用文章目录，新增或修改正文需提供密码；无真实文章时只展示空状态。
